...
def log_warning(message: str, details: Optional[Dict] = None):
    ...
    results["warnings"] += 1
    results["details"].append(
        {"passed": None, "message": message, "details": details or {}, "warning": True}
    )
...
def get_frontend_token() -> Optional[str]:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if token:
                log_result(
                    ...
                    {"token_preview": f"{token[:50]}...", "token_length": len(token)},
                )
                ...
                log_warning(
                    "Token endpoint returned 200 but no token in response",
                    {"response": data},
                )
                return None
        else:
        ...
        log_warning(
            "Frontend token endpoint not accessible",
            {
                "url": f"{FRONTEND_URL}/api/auth/token",
    ...
    try:
        ...
        if response.status_code in [200, 204]:
            if acao == origin or acao == "*":
                log_result(
                    True,
                    f"CORS preflight accepted for {origin}",
                    {
                ...
                log_result(
                    False,
                    f"CORS preflight: Expected '{origin}', got '{acao}'",
                    {
                        "status": response.status_code,
    ...
    try:
        ...
        if response.status_code == 200:
            if require_cors and not acao:
                log_warning(
                    f"Public endpoint accessible but missing CORS headers",
                    {
                        "endpoint": endpoint,
                        "status": response.status_code,
                    },
            ...
            log_result(
                False,
                f"Public endpoint failed: Status {response.status_code}",
                {
                    "endpoint": endpoint,
                    "status": response.status_code,
...
def test_protected_endpoint(token: str) -> None:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            log_result(
                False,
                f"Protected endpoint failed: Status {response.status_code}",
                {
                    "status": response.status_code,
                    "response": response.text[:200],
                },
...
def test_unauthorized() -> None:
    ...
    try:
        ...
        if response.status_code == 200:
            log_result(
                True,
                "Public endpoint accessible without auth (expected)",
                {
                    "status": response.status_code,
                    "Access-Control-Allow-Origin": acao,
                },
...
def test_invalid_token() -> None:
    ...
    try:
        ...
        if response.status_code in [401, 403]:
            ...
            log_warning(
                "Invalid token accepted (may be permissive for public endpoints)",
                {
                    "status": response.status_code,
                },
            )
            ...
            log_result(
                False, f"Unexpected status for invalid token: {response.status_code}"
            )
    except Exception as e:
...
def main():
    ...
    if not backend_ok:
        ...
        print(
            "  Start backend with: cd backend/mario-health-api && uvicorn app.main:app --reload"
        )
    else:
