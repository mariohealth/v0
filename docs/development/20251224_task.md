...
## Phase 2: Live Search Debugging & Auth Repair [COMPLETED]
- [x] **Identify Production Search Failure**
    - [x] Verify API connectivity vs UI rendering (Subagent)
    - [x] Confirm CORS preflight error with `Authorization` header
- [x] **Fix Search & Enable Auth**
    - [x] Update `api.ts` to use `fetchWithAuth` for all searches
    - [x] Implement CORS-safe "Smart Auth" fallback
    - [x] Fix data structure mapping and logging in `mario-smart-search.tsx`
    - [x] Verify type-safety with `npx tsc` (resolved regressions)

## Phase 3: Live Deployment [COMPLETED]
- [x] **Firebase Deployment**
    - [x] Fix `.env.firebase` configuration
    - [x] Build and Deploy to Production
    - [x] Consolidate routes and resolve static export conflicts
