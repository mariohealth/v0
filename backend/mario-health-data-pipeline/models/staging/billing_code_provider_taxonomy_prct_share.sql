...
JOIN {{ ref('npi_data_taxonomy_code_unpivot') }} AS t2
...
t3 AS (
  ...
  FROM t2
  JOIN `mario-mrf-data.test_arnaud.top_cpt_codes` AS top
  ON t2.hcpcs_cd = CAST(top.Code AS string)
),
...
SELECT *,
ROW_NUMBER() OVER (PARTITION BY Code  ORDER BY count_rows_per_taxonomy DESC) AS row_num,
FROM t4
ORDER BY 3, 6 DESC
