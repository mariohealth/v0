...
class FamilyService:
    ...
    async def get_procedures_by_family(self, slug: str) -> FamilyProceduresResponse:
        ...
        family_check = (
            self.supabase.table("procedure_family")
        ...
        result = self.supabase.rpc(
            "get_procedures_with_pricing", {"family_slug_input": slug}
        ).execute()
        ...
        if result.data:
            procedures = [
                Procedure(
                    ...
                    min_price=(
                        Decimal(str(p["min_price"])) if p.get("min_price") else None
                    ),
                    max_price=(
                        Decimal(str(p["max_price"])) if p.get("max_price") else None
                    ),
                    avg_price=(
                        Decimal(str(p["avg_price"])) if p.get("avg_price") else None
                    ),
